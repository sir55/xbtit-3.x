...
class archive {
  ...
  function parse_dir($dirname) {
    if ($this->options['storepaths'] == 1 && !preg_match('/^(\.+\/*)+$/', $dirname))
      $files = array (array ('name' => $dirname, 'name2' => $this->options['prepend'] .
        preg_replace('/(\.+\/+)+/', '', ($this->options['storepaths'] == 0 && strstr($dirname, '/')) ? substr($dirname, strrpos($dirname, '/') + 1) : $dirname), 'type' => 5, 'stat' => stat($dirname)));
    else
      $files = array ();
    $dir = @opendir($dirname);
  ...
  function download_file() {
    ...
    header('Content-Disposition: attachment; filename="'.(strstr($this->options['name'], '/') ? substr($this->options['name'], strrpos($this->options['name'], '/') + 1) : $this->options['name']).'"');
